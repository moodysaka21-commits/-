...
class _CounterScreenState extends State<CounterScreen> {
  ...
  Map<String, dynamic> get currentZekr => widget.azkarList[currentIndex];

  void _increment() {
    HapticFeedback.lightImpact();
    setState(() {
      if (currentCount < currentZekr['targetCount']) {
        currentCount++;
      } else {
  ...
  @override
  Widget build(BuildContext context) {
    final target = currentZekr['targetCount'] as int;
    final progress = target == 0 ? 0.0 : currentCount / target;
    final hasNext = currentIndex < widget.azkarList.length - 1;
      ...
      body: GestureDetector(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              Text(
                '${currentIndex + 1} من أصل ${widget.azkarList.length}',
              ...
              if ((currentZekr['benefit'] as String).isNotEmpty) ...[
                Container(
              ...
              const Text('اضغط للتسبيح، أو اسحب للشاشة للتنقل بين الأذكار', style: TextStyle(color: Colors.white38, fontSize: 12)),
              const SizedBox(height: 10),
            ],
          ),
        ),
